...
class PropNav(Guidance):
    ...
    def get_angle(self) -> float:
        ...
        if self._prev_los_vector_angle is None and self._gamma_deg is None:
            ...
            gamma_rate_deg = velocity_closing = velocity_interceptor = 0 # for printing
        else:
            velocity_closing =-(distance - self._prev_distance)
            velocity_interceptor = np.linalg.norm(self.interceptor.coords - self._prev_interceptor_coords)
            vc_2_vi = velocity_closing / velocity_interceptor if velocity_interceptor != 0 else 1
        ...
        msg = f"LOS {los_vector_angle:.1f}, LOS rate {los_rate_deg:.1f}, velocity_closing {velocity_closing:.1f}, "\
                f"velocity_interceptor {velocity_interceptor:.1f}, gamma_rate_deg {gamma_rate_deg:.1f}, gamma_deg {self._gamma_deg:.1f}"
        print(msg)
